var app =angular.module('facturacionApp');
  app.controller('MisProformasCtrl', function ($scope,$rootScope,Proformas_Services,Servicios_Generales,$window,$mdDialog) {
			$scope.selected=[];
			// -------------------------------------------------------MIS PROFORMAS------------------------------------------------------------
			//------------------------------------------------- LLENADO DE TABLA -----------------------------------------
			...
			$scope.get_tabla=function(){
				Proformas_Services.Proformas().Get().send($scope.query,success_tabla).$promise
			}
			...
			$scope.update_deducir=function(factura){
					Proformas_Services.Proformas().Update().send(factura).$promise.then(function(data){
			...
			//----------------------------------------------------- DETALLES DE FACTURA -----------------------------------------
				$scope.ver_detalles=function(factura){
					...
					var url=Servicios_Generales.server()+'proformas/'+factura.id+'.pdf';
            ...
            templateUrl: 'views/Dash/Proformas/Add/delete.html',
			...
			function DialogController_delete($scope,Proformas_Services,LxNotificationService,obj) {
				$scope.eliminar=function(){
					Proformas_Services.Proformas().Delete().send({id:obj.id}).$promise.then(function(data){
						if (data.respuesta==true) {
							$rootScope.$emit("actualizar_tabla", {});
							$mdDialog.hide();
            ...
            templateUrl: 'views/Dash/Proformas/Add/update.html',
            parent: angular.element(document.body),
            targetEvent: event,
            ariaLabel: 'Respuesta Registro',
            clickOutsideToClose: false,
			...
			function DialogController_update($scope,Proformas_Services,LxNotificationService,obj,Configuracion_Services,$localStorage,Inventario_Services,Facturacion_Services) {
			...
			//----------------------------------------------------- DETALLES DE FACTURA -----------------------------------------
    		$scope.detalles_fac=obj.productos;
    		$scope.totales=$localStorage.totales;
    		$scope.add_prod_fac=function(prod){
    			for (var i = 0; i < $scope.detalles_fac.length; i++) {
    				if ($scope.detalles_fac[i].codigo_prod==prod.codigo_prod) {
    					if ($scope.detalles_fac[i].cantidad_fac<$scope.detalles_fac[i].cantidad) {
    						$scope.detalles_fac[i].cantidad_fac++;
    						$scope.detalles_fac[i].total_fac=$scope.detalles_fac[i].total_fac=parseFloat(parseFloat($scope.detalles_fac[i].precio.replace('$','')).toFixed($scope.decimales)*$scope.detalles_fac[i].cantidad_fac).toFixed(2);
    					}
    					break;
    				}
    			}
    		...
    		$scope.guardar_factura=function(tipo_save_fac){
				...
				Proformas_Services.Proformas().Update().send({cliente:$scope.data,detalles:$scope.detalles_fac,totales:$scope.totales}).$promise.then(function(data){
					if (data.respuesta==true) {
						$scope.get_tabla();
						$scope.detalles_fac=[];
						$mdDialog.hide();
   ...
   app.controller('ProformasCtrl', function ($scope,$rootScope,$window, $localStorage,$mdDialog,Facturacion_Services,Servicios_Generales,Configuracion_Services,Inventario_Services,Proformas_Services,LxNotificationService ) {
			// -------------------------------------------------------VENDER------------------------------------------------------------//
			$scope.selected=[];
			$scope.decimales=2;
			$scope.cliente='NO';
    		...
    		$scope.guardar_factura=function(tipo_save_fac){
    			...
    			if ($scope.cliente=='SI') {
					...
					Proformas_Services.Proformas().Add().send({cliente:$scope.data,detalles:$scope.detalles_fac,totales:$scope.totales}).$promise.then(function(data){
						if (data.respuesta==true) {
							$scope.get_tabla();
							$scope.detalles_fac=[];
							$mdDialog.hide();
							LxNotificationService.success('Proformado Correctamente');
							$window.open(data.fac, '_blank');
						}
					},function(error){
						LxNotificationService.error('Ha ocurrido un error intentalo nuevamente :(');
